...
> 我已经把它从方案落成可操作链路。Task 同时维护执行、内容和发布三条状态；用户先补 Content Brief，再由系统检索当前租户有效知识并保留 source/chunk 引用；平台、显式品牌偏好、已晋升偏好、账号风格卡和任务覆盖会合并成冻结快照。任务页支持采用、退回和人工修订，修订会创建带 parent_copy_id 和 diff 的新版本。发布结果另表幂等保存，指标可后补；同一结构化风格信号至少三次正向证据才晋升为长期偏好。完整后端回归是 215 项，前端 9 项且生产构建通过。这里证明的是实现与隔离边界，真实采用率、内容质量和平台效果仍需要预发迁移、盲评和线上数据验证。
