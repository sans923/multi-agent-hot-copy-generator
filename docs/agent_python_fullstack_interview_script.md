...
> FastAPI 依赖为每个请求提供独立 Session，Service 层根据一个完整业务不变量划定事务边界：一组必须同时成功的数据库操作放在同一事务中，成功后 commit，异常时 rollback，最后关闭 Session 并把连接归还池。`flush` 只是把 SQL 发到数据库以获得主键等结果，不等于提交。
>
> Agent 长任务不适合持有长事务，因为模型和第三方 API 可能等待几十秒，会长期占用连接和锁。我会把任务状态、Checkpoint 和阶段结果拆成短事务；跨数据库与消息队列的一致性用幂等键、状态机和 Outbox，而不是让一个事务跨越外部网络调用。

### Q4.1：主键、外键、唯一约束分别解决什么？

> 主键唯一标识一行；外键保护表之间的引用完整性；唯一约束保证某个业务键不能重复。API 校验只能保护当前入口，唯一约束可以抵抗脚本、Worker 和并发请求的重复写入。对于多租户业务，唯一键通常还要包含 `user_id` 或 `tenant_id`，避免不同用户互相冲突。

### Q4.2：什么是索引，为什么不是越多越好？

> 索引是额外维护的有序数据结构，用空间和写入成本换查询速度。适合高频过滤、关联和排序字段。索引多了会增加存储，降低 INSERT、UPDATE 和 DELETE 性能；低选择性或顺序错误的联合索引也可能没有收益。我会先拿到真实 SQL 和数据分布，用 EXPLAIN、慢查询日志和 P95 延迟确认瓶颈，再设计联合索引，而不是看到慢就盲目加索引。

### Q4.3：什么是数据库隔离级别？

> 隔离级别控制并发事务彼此能看到什么，用来权衡一致性和并发。常见异常有脏读、不可重复读和幻读。MySQL InnoDB 常使用 Repeatable Read 和 MVCC 提供一致性快照，但 MVCC 不代表没有锁；UPDATE、唯一约束和 `SELECT FOR UPDATE` 仍会产生锁。关键写入还要配合原子条件更新、乐观锁或悲观锁。

### Q4.4：ORM 和 SQL 应该怎么选？

> ORM 适合常规 CRUD、关系映射和统一事务管理，能提高可读性，但不能代替 SQL 知识。复杂聚合、批量处理或性能敏感查询可能用 Core 或手写参数化 SQL 更清晰。无论使用哪种方式，我都会观察最终 SQL，避免懒加载产生 N+1，并只查询接口真正需要的列。

### Q4.5：如何排查一条慢 SQL？

> 我先确认慢的是数据库而不是模型或网络，然后获取带真实参数形状的 SQL，查看慢查询日志和 EXPLAIN：是否全表扫描、扫描行数、使用哪个索引、是否额外排序或临时表。再检查 N+1、返回列过多、深 OFFSET、锁等待和连接池等待。修改后用相同数据量复测 P95/P99，并观察写入成本，不能只看单次本机耗时。

### Q4.6：ORM 加了字段，为什么生产数据库还会报列不存在？

> ORM 类只是应用期望的 Schema，`create_all()` 通常不会 ALTER 已存在的表。生产要用 Alembic 等迁移系统维护版本，先在目标数据库审查和测试 SQL，再由单一迁移任务升级；多 Web Worker 不应同时执行 DDL。迁移完成后还要检查实际列、版本表并执行原始 ORM 查询。备份也必须做恢复演练，否则只有备份文件不等于可恢复。
...
> 我已经把它从方案落成可操作链路。Task 同时维护执行、内容和发布三条状态；用户先补 Content Brief，再由系统检索当前租户有效知识并保留 source/chunk 引用；平台、显式品牌偏好、已晋升偏好、账号风格卡和任务覆盖会合并成冻结快照。任务页支持采用、退回和人工修订，修订会创建带 parent_copy_id 和 diff 的新版本。发布结果另表幂等保存，指标可后补；同一结构化风格信号至少三次正向证据才晋升为长期偏好。完整后端回归是 215 项，前端 9 项且生产构建通过，三份迁移也已在真实 MySQL 8.0.46 可重入执行。这里证明的是实现与隔离边界，真实采用率、内容质量和平台效果仍需要盲评和线上数据验证。
...
### 面试官：ORM 已有字段，但线上报 Unknown column，你怎么修？
