...
1. **[2026-08-22 已闭环] 任务可靠执行**：创建与人工 retry 已由 MySQL 持久 Job 和独立 Worker 承载，具备原子入队、租约、heartbeat、业务写 fencing、有限重试、dead/revive 和过期回收；LangGraph checkpoint 与 Job 消费职责分离。仍待入口用户级幂等、背压/指标和物理进程故障演练。
...
**[合理判断]** 对作品集来说，先把“可靠生成 + 可解释审核”讲清楚，比立即接多个发布平台更划算。持久任务执行与启动恢复已在 2026-08-22 落地；下一项业务 P1 是创建入口幂等、队列背压和可观测性。发布平台 OAuth 和效果回流需要真实账号及运营数据，应后续分阶段实现。

你可以用自己的话这样理解：现在系统已经能“稳定排队、接单、写稿、审稿、交稿”，但还没有完整做到“重复提交结果回放、按意见改稿、真正发布、收集效果、再优化下一篇”。
...
3. **[2026-08-22 已修复/P1]** 长时生成已迁到持久 Job 与独立 Worker；Web 重启不再决定 Job 是否存在，认领、续租、重试和 stale write 均有数据库约束。
4. **[本轮已修复/P1]** 已用 uv 托管 Python 3.11.9 重建 `.venv`，提交 149 个包的精确锁文件，并在该环境中完成全量 pytest：`113 passed, 6 warnings`。
5. **[2026-08-22 已修复/P1]** Compose 不再提供数据库明文默认密码，应用和 root 密码均为必填环境变量。
...
11. **[2026-08-22 已修复/P1]** 首次执行通过 dedupe key、唯一约束、条件认领和 lease token 原子化；业务 Session 用 token 行锁阻止旧执行写回。HTTP 创建入口自身的用户级幂等键仍待补。
...
- 已完成：将长任务迁到具备持久化、重试、租约和 fencing 的数据库队列与独立 Worker；
- 已完成：移除 Compose 中可被误用的数据库默认密码；
- 已完成：为首次任务执行增加数据库原子认领，并补同 key 并发入队与 stale lease 测试；
...
6. **[2026-08-22 已完成]** 首次任务原子认领、durable Session fencing 与真实 MySQL 同 key 并发验证已完成；下一步补入口幂等和局部吞吐/故障基准。
7. **[2026-08-22 已完成]** 将长任务移到持久队列，并实现租约回收、幂等认领、有限重试和独立 Worker；物理 kill/restart 演练仍待补；
...
我做的是一个多智能体热点文案生成系统。用户通过 FastAPI 提交平台、文案需求和执行模式，系统把 Task 与 Job 同事务持久化，由独立 Worker 执行。核心流程拆成需求理解、文案创作和审核优化三个 Agent；Agent 负责决策，Skill 负责搜索、RAG、生成、合规检查和保存，PipelineState 负责在阶段间传递状态。项目同时支持自研 native 编排和 LangGraph，以及 fixed、agentic、lead 三种模式。长时任务具备租约、heartbeat、fencing、有限重试和 dead/revive；我也如实说明创建入口幂等、队列背压/指标、定向改稿、真实发布和效果回流仍需继续完善。
...
## 32.28 持久任务队列、独立 Worker 与前端资源闭环（2026-08-22）

**原始问题与触发场景：** 前一轮已经修复知识页请求反馈环，但仍有四项未闭环：Toast 自动移除计时器在 Provider 卸载时未清理、真实浏览器长期请求曲线未测、长时生成仍依赖 Web 进程、当前分支因包含另一个未推送提交而未同步远端。实现持久队列后的首轮审查又发现硬编码数据库密码、编排失败被错误确认完成、租约无 fencing/heartbeat、最终尝试崩溃后永久 processing、并发幂等先查后插和首次 `create_all()` 未进入迁移锁等问题。

**问题原因：** `BackgroundTasks` 与 Web 进程同生命周期，不能提供跨进程消费、崩溃恢复和有限重试；只用 `worker_id/locked_at` 无法阻止旧 Worker 覆盖新租约；先查后插在并发下仍会触发唯一键冲突；编排入口吞掉异常后，队列看不到失败。Compose 中明文默认密码还会覆盖 `.env`。前端方面，稳定 Context 已消除请求环，但 timeout handle 仍需要生命周期所有权。

**解决方案与修改文件：** 新增 `TaskExecutionJob`、`task_execution_queue.py`、MySQL 可重入建表/加列 SQL 和 `scripts/task_worker.py`；Task 创建与 start Job 同事务提交，resume retry 写入持久队列。认领使用条件更新、唯一 `lease_token` 与 attempt 组成 fencing token，长任务由独立 Session 周期续租；完成、失败和续租都校验当前 token，最终过期尝试由 reaper 标记 dead。失败结果和异常重新传播到队列并执行最多三次重试；dead revive 使用 `WHERE status='dead'` 条件更新，不能覆盖新认领。共享 heartbeat Event 会传入编排层；外层及 durable LangGraph 每节点 Session 都在 ORM flush 和 bulk update/delete 前，用同一业务事务 `SELECT ... FOR UPDATE` 校验并锁住当前 Job token，失租执行不能继续提交业务写。并发唯一键冲突在 savepoint 后结束 MySQL REPEATABLE READ 快照并读取既有 Job。Compose 增加一次性 migrate 与独立 worker，顺序固定为 MySQL healthy → migrate 成功 → app healthy → worker；密码改为必填环境变量，Web Worker 不再迁移 MySQL。`ToastContext.tsx` 用 Map 跟踪 timeout，移除 Toast 或 Provider 卸载时清理。

**测试方法与实际结果：** Toast 先以 RED 证明卸载后仍残留计时器，修复后相关 3 项通过。队列多轮 RED 覆盖模型缺失、异常语义、租约、dead revive、生产迁移顺序和业务 Session fencing；最终业务 fencing/队列聚焦 `31 passed`，完整后端 `238 passed, 9 warnings in 26.71s`；前端 `12 passed`，TypeScript/Vite 生产构建 62 modules。真实 MySQL 初始化退出 0，`lease_token` 列已查询确认；8 个线程并发写同一 dedupe key，实际结果为 1 个 Job ID、1 行、0 错误，验证数据随后清理。真实 Worker 对不支持 Job 实际执行 3 次后进入 dead，并释放 worker/token，测试 Job随后清理；最终 Web 健康检查 HTTP 200，Worker 以独立进程运行，UTF-8 日志可正常显示中文。此前真实任务 9 已由独立 Worker 完成并保存 2 版文案。Compose `config --quiet` 退出 0，仅因沙箱无权读取用户 Docker config 输出权限警告。

**真实浏览器曲线：** in-app browser 因当前面板挂载失败连续三次在 120 秒超时，未伪报成功；随后启动本机 headless Chrome，经真实登录表单进入 `/knowledge`。由于 React 18 StrictMode 加一次显式导航，`/api/v1/memory/insights` 初始共 4 次；0、30、60、90、120 秒采样计数始终为 4，页面 URL、标题和正文均稳定，确认没有长期请求增长。临时 Chrome 配置与验证脚本已清理。

**缺点、代价与预期效果：** 数据库队列现在具备持久化、租约回收、有限重试、业务写 fencing 和独立 Worker 的实际闭环，但仍是 1 秒轮询的单表实现，没有优先级、全局/用户并发闸门、队列时延指标或管理面板。当前关系库写入由同事务 token 行锁保护，最终索引已走 Outbox；极端网络分区下任意第三方副作用仍不能宣称 exactly-once，必须继续依赖提供方幂等键或结果查询。物理杀死真实 LLM Worker 后的完整恢复演练本轮未执行，属于尚未验证的预期效果。创建 Task 与 Job 已原子，但创建任务 HTTP 入口仍没有用户作用域的请求幂等键，重复点击仍可能创建两个不同 Task。

**面试时怎么讲：** “我没有把 BackgroundTasks 直接换成一个 while 循环就称为可靠队列。API 只在同一事务写 Task 和 Job，独立 Worker 用条件更新认领；每次租约生成 token，续租、完成和失败都带 token 与 attempt 做 fencing，崩溃后过期租约可回收，最后一次则进入 dead。审查发现异常被吞、并发快照和明文密码后，我用失败测试、真实 MySQL 八线程和实际三次重试逐项封闭。边界也明确：这是数据库队列，不是 exactly-once 消息平台，下一步是入口幂等、背压与可观测性。”

**[下一个最值得处理的 P1]** 给创建任务 API 增加 `(user_id, idempotency_key)`、请求摘要和结果回放，并为 Worker 增加全局/用户并发上限、队列等待时间/失败率指标和管理面板；部署侧继续把轻量 SQL 升级为 Alembic 版本化迁移。
...
| 2026-08-22 | 完成 Toast 资源清理、真实浏览器曲线与持久任务 Worker | 清理 Toast timeout；新增数据库 Job、原子入队、lease token/fencing、heartbeat、业务 Session 行锁、重试/dead/revive、一次性 migrate、独立 Worker、Compose 环境密码与 MySQL 队列表迁移 | RED/GREEN；后端 `238 passed, 9 warnings`；前端 `12 passed`、构建 62 modules；Chrome 120 秒请求数稳定为 4；MySQL 8 线程同键 1 行 0 错误；真实 Worker 3 次失败后 dead；Compose config、真实迁移和最终健康检查成功 | 第 32.28、33 节及知识手册、面试话术、情景题、对话收件箱 | 已完成本轮四项；物理进程故障演练、入口幂等、背压/指标和第三方 exactly-once 边界仍待验证 |
